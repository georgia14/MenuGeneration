...
rm -rf $_file
cat $_input | grep "${_triggerArray[$iT]} " | awk '{ if($obj==$_obj[0]) {print $2} else if($obj==$_obj[1]) { print $2 } else if($obj==$_obj[2]) { print $2 } }' > $_file &
cat $_file
...
# threshold error low
rm -rf $_file
cat $_input | grep "${_triggerArray[$iT]} " | awk '{ if($obj==$_obj[0]) {print $7} else if($obj==$_obj[1]) { print $8 } else if($obj==$_obj[2]) { print $10 } }' > $_file &
cat $_file
...
rm -rf $_file
cat $_input | grep "${_triggerArray[$iT]} " | awk '{ if($obj==$_obj[0]) { print $8} else if($obj==$_obj[1]) { print $9 } else if($obj==$_obj[2]) { print $11 } }' > $_file &
cat $_file
...
#echo "g->GetYaxis()->SetRangeUser((g->GetMinimum()-10.),(g->GetMaximum()+10.));" >> $_macro
